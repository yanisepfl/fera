# Emissions — where FERA comes from, and why it can't out-print revenue
...
| **5%** | **Traders** | Pro-rata to the fees they paid — a small rebate. Halved from an earlier 10% to 5%: it's the safest slice (it roughly doubles the margin that makes wash-farming unprofitable) at no cost to routing, since routers choose pools by depth, not by rebate. |
...
- **Staking can boost your LP slice** up to ~2×, but that's a re-weighting of the fixed pool, not
  new tokens (see [Rewards & vesting](./rewards-and-vesting.md)).
...
- **`cap(t)`** is a fixed supply schedule — a logistic curve over the 900M usage bucket, spread
  across ~4 years (peaking around 9.73M FERA per week at the top of the curve). This is the
  ceiling from the *supply* side.
- **`β × revenue`** is the *revenue* bound. `β` is 0.8, and `revenue` is the protocol's actual fee
  revenue that epoch, valued in FERA at a manipulation-resistant time-weighted price. This is the
  ceiling from the *earnings* side.

Emissions are the **lower of the two.** When the revenue bound sits below the supply cap — which is
the case in the early life of the protocol — **issuance is gated by revenue.** FERA literally
cannot emit more value than it earned. We call this a **"dividend of activity, not a subsidy":** the
token is a claim on real usage, not a promise printed against the future.

The Transparency page draws all three lines — the supply cap, the revenue bound, and what actually
got emitted (which can only ever touch the *lower* of the two). You can watch the gate work.
...
> emissions are small.** In the base case, only about **11%** of the 900M usage bucket emits over
> the first four years. The token can be worth very little at the start.
...
dividend of activity that grows as real usage grows — not a launch subsidy and not the reason to
show up.

The early liquidity that the protocol needs is seeded from the **treasury war-chest**, not from
usage emissions. That's the only design consistent with the "emissions ≤ revenue" rule.

## Why this design

Most token emissions are a subsidy: the protocol prints tokens to rent liquidity, the tokens
inflate faster than revenue, and the price bleeds. FERA inverts that: **the token can never inflate
faster than the protocol earns.** It's less exciting on day one — and far more honest about what the
token actually is.
