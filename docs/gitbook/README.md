...
weekend-arbitrage trading flow the fee it is actually worth — so the volatility that drains
ordinary liquidity providers pays FERA's providers instead. It also ships a managed vault, so you
can provide liquidity to a memecoin pool or a tokenized-stock pool without running a keeper.
...
A Uniswap v4 **hook** sets the swap fee *per trade, based on live market conditions* — high when
flow is toxic (violent memecoin moves, bots, weekend stock-token drift), near-zero when flow is
benign. Traders always get to swap; they just pay what the moment is worth, and that fee goes to
liquidity providers. A managed **vault** runs a transparent, rule-based strategy on top, so you can
be a liquidity provider with one deposit.
...
| [How the dynamic fee works](how-fees-work.md) | The mechanism — MEME volatility fee and RWA weekend-drift fee, with the actual numbers. |
