...
| Slice | Who | Why |
|-------|-----|-----|
| **85%** | **Liquidity providers** | Pro-rata to the fees your vault shares earned. Liquidity is FERA's one bottleneck, so the split leans hard toward it. |
...
| **10%** | **Treasury** | A protocol-owned war-chest that compounds into protocol-owned liquidity. This is the lever that helps seed liquidity when the token is young. |
...
subsidy.** It is not a promise printed against the future.
...
claim on real activity that grows as usage grows. They are not a launch subsidy and not the reason to
show up.
